...
$qry = "SELECT SUM(idd.weices * idd.rate + idd.charges) as total FROM `invoice` i,`invoice_detail` idd WHERE i.no = idd.ref";
$run = mysqli_query($con,$qry) or die(mysqli_error($con));
$data = mysqli_fetch_array($run);
$custInvTot_ALL = floatval($data['total']);

$qry = "SELECT SUM(amount) as total FROM `payments_recv` WHERE 1";
//echo $qry;
$run = mysqli_query($con,$qry);
$data = mysqli_fetch_array($run);
...
$TOTAL_PAYABLE = 0;
$qry = "SELECT b.* FROM `bill` b,`vendor` v  WHERE v.id = b.vendor and  v.name = 'FACTORY'";
$run = mysqli_query($con,$qry) or die(mysqli_error($con));
$total_balance = 0;
while($data = mysqli_fetch_array($run))
{
...
$qry = "SELECT b.* FROM `bill` b,`vendor` v  WHERE v.id = b.vendor and  v.name = 'BLOCK'";
$run = mysqli_query($con,$qry) or die(mysqli_error($con));
$total_balance = 0;
while($data = mysqli_fetch_array($run))
{
...
$closingBalance = $recable_ALL - $TOTAL_PAYABLE;
echo $closingBalance;
?>